...
using SkyGroundLabs.Net.Google.Calendars.ResponseResults;
...
namespace SkyGroundLabs.Net.Google.Calendars
{
	public class GoogleCalendars
	{
		...
		private IEnumerable<CalendarEvent> _getEventsByQueryString(string queryString, DateTime startTime, DateTime endTime)
		{
			...
			try
			{
                var service = new CalendarService(GoogleAPISettings.CalendarAppName);
				...
				for (var i = 0; i < myResultsFeed.Entries.Count; i++)
				{
					var entry = myResultsFeed.Entries[i] as EventEntry;
		...
		private CalendarEvent _getEventByID(string eventId, string searchText)
		{
		    ...
		    return list.FirstOrDefault(w => w.EventID == eventId);
		}
		#endregion
	}
}
